...
De ingest staat standaard uit. Een beheerder met `wallboards.manage` en voltooide 2FA configureert deze normaal via **Admin > Wallboards > OBS live-uitzending**. Bij de eerste inschakeling maakt de rootbroker server-side een cryptografisch willekeurige Stream Key; de browser levert of genereert die key nooit. Neem de echte key nooit op in documentatie, logs of supportberichten. Gebruik een eigen DNS-naam met een publiek vertrouwd certificaat. D.I.S. vraagt of vernieuwt dat certificaat niet zelf: de certificaatbestanden moeten vooraf door root zijn beheerd en aanwezig zijn. Onderstaand `.env`-contract blijft uitsluitend beschikbaar voor herstel en geavanceerd serverbeheer:
...
# Door de server gegenereerd; alleen bij herstel handmatig beheren.
...
Opslaan in het portaal wijzigt de beheerde serverconfiguratie atomisch en herlaadt de media-runtime en PHP-FPM pas nadat de volledige configuratie is gevalideerd. Een bestaande Stream Key blijft bij uitschakelen en herconfigureren behouden. Alleen wanneer een ingeschakelde configuratie nog geen geldige key heeft, maakt de rootbroker er een aan. Een logische no-op onderbreekt OBS niet. De actuele Stream Key is alleen via een expliciete actie in het wallboardportaal op te halen; de gewone statuscontrole bevat hem nooit. Wisselen vereist een afzonderlijke gevaarsbevestiging, trekt de oude key direct in en meldt duidelijk dat OBS opnieuw moet verbinden. Als activatie of de bevestiging aan het portaal niet aantoonbaar slaagt, wordt de vorige configuratie en runtime teruggezet of stoppen de live-streamservices fail-closed.

`PUBLIC_HOST` moet exact overeenkomen met de certificaathostnaam. Het portaal accepteert voor certificaat en private key uitsluitend veilige absolute paden onder `/etc/letsencrypt/live/` of `/etc/ssl/`; het leest of uploadt die bestanden niet. De private-keybron moet uitsluitend voor root leesbaar zijn. De rootbroker volgt eventuele symlinks en controleert eigenaar, rechten, geldigheidsperiode, publieke vertrouwensketen, serverdoel, hostnaam en of certificaat en sleutel bij elkaar horen. Bestaande installaties met een ander root-beheerd absoluut certificaatpad blijven voor updates en herstel ondersteund, maar nieuwe portaalconfiguratie kan alleen de genoemde paden kiezen. Kies in OBS bij **Stream** de service **Custom...** (Custom Service) en gebruik:
