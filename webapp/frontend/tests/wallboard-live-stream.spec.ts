...
test('shows the RTMPS server and retrieves the Stream Key only through an explicit action', () => {
  ...
  expect(EDITOR_SOURCE).toContain('disabled={keyActionBusy || !keyManagementReady || status?.stream_key_configured !== true}');
  ...
  expect(EDITOR_SOURCE).not.toContain('onChange(');
...
test('rotates the Stream Key behind a danger confirmation and gives explicit OBS recovery guidance', () => {
  ...
  expect(EDITOR_SOURCE).toContain('disabled={keyActionBusy || !keyManagementReady}');
