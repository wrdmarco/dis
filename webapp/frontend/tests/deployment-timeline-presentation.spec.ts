...
import { presentDeploymentTimelineItem } from '../src/features/deployments/deploymentTimelinePresentation';
import type { DeploymentTimelineItem } from '../src/types/api';

test('shows who added a control-room note without repeating the actor', () => {
  const presentation = presentDeploymentTimelineItem(timelineItem({
...
function timelineItem(overrides: Partial<DeploymentTimelineItem>): DeploymentTimelineItem {
  return {
    id: 'timeline-1',
    type: 'internal_notes',
    label: 'Meldkamer kladblok',
