...
test('renders all five workflow stages and uses only versioned workflow mutations', () => {
  ...
  expect(adminPage).toContain('<DeploymentRequestWorkflowStudio onDirtyChange={setDeploymentRequestWorkflowDirty} />');
  expect(adminPage).toContain("'/admin/deployment-form/config'");
  ...
  for (const endpoint of [
    ...
    '/admin/deployment-request-workflow/draft',
    '/admin/deployment-request-workflow/validate',
    '/admin/deployment-request-workflow/simulate',
    '/admin/deployment-request-workflow/publish',
    '/admin/deployment-request-workflow/restore',
  ]) {
...
test('blocks a forms-tab switch while the deploymentRequest workflow has unsaved changes', () => {
  let confirmations = 0;
  const blocked = adminTabChangeAllowed({
    currentTab: 'deploymentRequest',
    nextTab: 'deploymentForm',
    deploymentRequestWorkflowDirty: true,
    confirmLeave: () => {
      confirmations += 1;
      return false;
    },
...
test('keeps technical option values and rule conditions stable when a label changes', () => {
  const objectCategory: DeploymentRequestWorkflowField = {
    ...field('object_category', 'Objectsoort', 'object', 'select'),
    options: [
  ...
  const rule: DeploymentRequestWorkflowPriorityRule = {
    id: 'regel_geslacht',
    label: 'Mannelijk',
...
test('clears a linked proposal when editing its profile makes the rule incompatible', () => {
  const profile: DeploymentRequestWorkflowDeploymentProfile = {
    ...createWorkflowDeploymentProfile([]),
    id: 'person-low',
    subject_types: ['person'],
    priorities: ['low'],
  ...
  const rule: DeploymentRequestWorkflowPriorityRule = {
    ...createWorkflowPriorityRule([], fields),
    id: 'person-low-rule',
    subject_types: ['person'],
    priority: 'low',
  ...
  const compatible = updateWorkflowDeploymentProfile(
    configuration,
    profile.id,
    (current) => ({ ...current, label: 'Nieuwe naam' }),
  );
  expect(compatible.priority_rules[0]?.deployment_profile_id).toBe(profile.id);

  const incompatible = updateWorkflowDeploymentProfile(
    configuration,
    profile.id,
    (current) => ({ ...current, priorities: ['urgent'] }),
  );
  expect(incompatible.priority_rules[0]?.deployment_profile_id).toBeNull();
});

test('keeps decision rules focused per subject without hiding shared global ordering', () => {
  const subjectOnlyFields = fields.filter((candidate) => candidate.scope !== 'common');
  const animalRule = createWorkflowPriorityRuleForSubject([], subjectOnlyFields, 'animal');
  const objectRule = createWorkflowPriorityRuleForSubject([animalRule], subjectOnlyFields, 'object');
  expect(animalRule.subject_types).toEqual(['animal']);
  expect(animalRule.conditions[0]?.field_key).toBe('animal_name');
  expect(objectRule.subject_types).toEqual(['object']);
  expect(objectRule.conditions[0]?.field_key).toBe('object_category');

  const personFirst: DeploymentRequestWorkflowPriorityRule = {
    ...createWorkflowPriorityRule([], fields),
    id: 'person_first',
    subject_types: ['person'],
  };
  const shared: DeploymentRequestWorkflowPriorityRule = {
    ...createWorkflowPriorityRule([personFirst], fields),
    id: 'shared',
    subject_types: ['person', 'animal'],
  };
  const animalOnly: DeploymentRequestWorkflowPriorityRule = {
    ...createWorkflowPriorityRule([personFirst, shared], fields),
    id: 'animal_only',
    subject_types: ['animal'],
  };
  const personLast: DeploymentRequestWorkflowPriorityRule = {
    ...createWorkflowPriorityRule([personFirst, shared, animalOnly], fields),
    id: 'person_last',
    subject_types: ['person'],
  };
  const rules: DeploymentRequestWorkflowPriorityRule[] = [personFirst, shared, animalOnly, personLast];

  expect(workflowPriorityRulesForSubject(rules, 'person').map((rule) => rule.id))
    .toEqual(['person_first', 'shared', 'person_last']);
  expect(workflowPriorityRulesForSubject(rules, 'animal').map((rule) => rule.id))
    .toEqual(['shared', 'animal_only']);

  const moved = moveWorkflowPriorityRuleForSubject(rules, 'shared', 'person', 1);
  expect(moved.map((rule) => rule.id)).toEqual(['person_first', 'person_last', 'animal_only', 'shared']);
  expect(workflowPriorityRulesForSubject(moved, 'animal').map((rule) => rule.id))
    .toEqual(['animal_only', 'shared']);
  expect(moveWorkflowPriorityRuleForSubject(rules, 'person_first', 'person', -1)).toBe(rules);
});

test('sends timezone-qualified datetime values from rule and simulation controls', () => {
  const iso = '2026-07-26T10:30:00.000Z';
  const local = deploymentRequestWorkflowDateTimeLocalValue(iso);

  expect(local).toMatch(/^2026-07-26T\d{2}:30$/);
  expect(deploymentRequestWorkflowDateTimeIsoValue(local)).toBe(iso);
  expect(deploymentRequestWorkflowDateTimeLocalValue('ongeldig')).toBe('');
  expect(deploymentRequestWorkflowDateTimeIsoValue('')).toBe('');

  const studio = source('../src/features/admin/DeploymentRequestWorkflowStudio.tsx');
  expect(studio.match(/deploymentRequestWorkflowDateTimeLocalValue\(value\)/g)).toHaveLength(2);
  expect(studio.match(/deploymentRequestWorkflowDateTimeIsoValue\(event\.target\.value\)/g)).toHaveLength(2);
});

test('reuses the conflict-safe workflow studio for the separate priority decisions page', () => {
  const page = source('../src/features/admin/DeploymentRequestPriorityDecisionsPage.tsx');
  const studio = source('../src/features/admin/DeploymentRequestWorkflowStudio.tsx');
  const route = source('../app/prioriteitsbesluiten/page.tsx');

  expect(page).toContain('<DeploymentRequestWorkflowStudio mode="decisions" />');
  expect(route).toContain('<ProtectedShell {...webRouteAccess.priorityDecisions}>');
  expect(studio).toContain("mode === 'decisions' ? 'rules' : 'questions'");
  expect(studio).toContain("['rules', 'profiles', 'versions']");
  expect(studio).toContain("['questions', 'bindings', 'versions']");
  expect(studio).toContain('decisionSubjectRail');
  expect(studio).toContain('<PriorityRail rules={visibleRules}');
  expect(studio).toContain('rule.subject_types.length > 1 || (adjacentRule?.subject_types.length ?? 0) > 1');
  expect(studio).toContain("profile.team_ids.includes(team.id) ? 'Aan' : 'Uit'");
  expect(studio).toContain('Publiceren activeert de volledige opgeslagen conceptconfiguratie');
  expect(studio).toContain('expected_revision: workflow.data.draft.lock_version');
  expect(studio).toContain('Mijn lokale versie behouden');
});
...
function field(
  ...
  type: DeploymentRequestWorkflowField['type'],
