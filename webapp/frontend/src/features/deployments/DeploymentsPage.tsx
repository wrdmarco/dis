...
import type { ConfigurableFormField, Deployment, DeploymentFormLayoutItem, DroneFlightContext, Team, User } from '../../types/api';
import { RealtimeBridge } from '../realtime/RealtimeBridge';
import { deploymentStatusPayload } from './deploymentStatusFlow';
...
type DeploymentPageMode = 'active' | 'archive';

const activeDeploymentStatuses: Deployment['status'][] = ['draft', 'active', 'dispatching', 'in_progress'];
const archiveDeploymentStatuses: Deployment['status'][] = ['resolved', 'cancelled'];

export function DeploymentsPage({ mode = 'active' }: { mode?: DeploymentPageMode }) {
  const { hasPermission } = useAuth();
  const deployments = useApiResource<Deployment[]>(deploymentListPath(mode));
  const deploymentList = filterDeploymentsForMode(deployments.data ?? [], mode);
  const activeCount = deploymentList.filter((deployment) => ['active', 'dispatching', 'in_progress'].includes(deployment.status)).length;
  const draftCount = deploymentList.filter((deployment) => deployment.status === 'draft').length;
  const criticalCount = deploymentList.filter((deployment) => deployment.priority === 'critical').length;
  const resolvedCount = deploymentList.filter((deployment) => deployment.status === 'resolved').length;
  const cancelledCount = deploymentList.filter((deployment) => deployment.status === 'cancelled').length;
  const pageTitle = mode === 'archive' ? 'Inzetarchief' : 'Inzetten';
  const emptyText = mode === 'archive' ? 'Geen afgeronde of geannuleerde inzetten gevonden.' : 'Geen actieve inzetten of concepten gevonden.';
  const canManageDeployments = hasPermission('deployments.manage');

  return (
    <div className="page-stack deployment-page">
      <RealtimeBridge onOperationalEvent={() => void deployments.silentReload()} />
      <Panel
        title={pageTitle}
        action={mode === 'active' && canManageDeployments ? (
          <Link className="primary-button" href="/aanvragen/new">
            <Plus size={16} /> Nieuwe aanvraag
          </Link>
        ) : null}
      >
        <ResourceState loading={deployments.loading} error={deployments.error} empty={false}>
          <div className="deployment-list-view">
            <div className="deployment-list-summary">
              {mode === 'archive' ? (
                <>
                  <SummaryMetric label="Archief" value={String(deploymentList.length)} />
                ...
                <>
                  <SummaryMetric label="Openstaand" value={String(deploymentList.length)} />
            ...
            {deploymentList.length > 0 ? (
              <div className={mode === 'archive' ? 'deployment-card-grid deployment-card-grid--archive' : 'deployment-card-grid'}>
                {deploymentList.map((deployment) => (
                  <DeploymentCard deployment={deployment} mode={mode} key={deployment.id} />
                ))}
              </div>
            ) : (
...
function filterDeploymentsForMode(
  deployments: Deployment[],
  mode: DeploymentPageMode,
): Deployment[] {
  const statuses = mode === 'archive' ? archiveDeploymentStatuses : activeDeploymentStatuses;

  return deployments.filter((deployment) => statuses.includes(deployment.status));
}

function deploymentListPath(mode: DeploymentPageMode): string {
  const statuses = mode === 'archive' ? archiveDeploymentStatuses : activeDeploymentStatuses;

  return `/deployments?status=${statuses.join(',')}`;
}

function DeploymentCard({ deployment, mode }: { deployment: Deployment; mode: DeploymentPageMode }) {
  return (
    <Link className={`deployment-card deployment-card--${deployment.status}`} href={`/inzetten/${deployment.id}`}>
      <header>
        <span className="deployment-card__reference">{deployment.reference}</span>
        <div className="deployment-card__badges">
          <StatusPill value={priorityLabel(deployment.priority)} tone={deployment.priority === 'critical' ? 'bad' : deployment.priority === 'high' ? 'warn' : 'neutral'} />
          <StatusPill value={deploymentStatusLabel(deployment.status)} tone={deploymentTone(deployment.status)} />
        </div>
      </header>
      <strong>{deployment.title}</strong>
      <div className="deployment-card__meta">
        <MetaLine icon={<MapPin size={15} />} value={deployment.location_label ?? 'Geen opkomstlocatie'} />
        <MetaLine icon={<Users size={15} />} value={deploymentTeamsLabel(deployment)} />
        <MetaLine icon={<RadioTower size={15} />} value={deployment.coordinator?.name ?? 'Geen coordinator'} />
        <MetaLine icon={<Clock size={15} />} value={mode === 'archive' ? `Gesloten: ${formatDate(deployment.closed_at)}` : `Geopend: ${formatDate(deployment.opened_at)}`} />
      </div>
    </Link>
  );
}
...
function deploymentTone(status: Deployment['status']): 'neutral' | 'good' | 'warn' | 'bad' {
  switch (status) {
    case 'active':
    case 'dispatching':
    case 'in_progress':
...
export function DeploymentForm(props: {
  ...
  onChange: (updater: (current: DeploymentFormState) => DeploymentFormState) => void;
}) {
  const { api } = useAuth();
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      ...
      setFlightContextError(null);
      try {
        const response = await api.post<DroneFlightContext>('/deployments/flight-context-preview', {
          latitude,
          longitude,
  ...
  const activeLayout = deploymentFormLayout(layout, customFields)
    .flatMap((item) => {
      if (item.key !== 'status') {
        return [item];
      }
  ...
  return (
    <form className="form-grid" onSubmit={onSubmit}>
      ...
      <div className="actions-row form-grid__wide">
        ...
        <button className="primary-button" type="submit" disabled={saving || hasMissingRequiredFixedDeploymentInput(activeLayout, form, enforceConfiguredRequiredFixedInputs)}>
...
function DeploymentFormBlock(props: {
  item: DeploymentFormLayoutItem;
  form: DeploymentFormState;
  users: User[];
  teams: Team[];
  customFields: ConfigurableFormField[];
  ...
  const required = deploymentLayoutItemRequired(props.item, props.enforceConfiguredRequiredFixedInputs);
  const requiredMark = required ? ' *' : '';

  switch (props.item.key) {
    case 'section_deployment':
      return <FormSectionTitle title="Inzetgegevens" />;
    case 'title':
      return (
    ...
    case 'priority':
      return (
        <label className={wideClass}>
          ...
          <select value={props.form.priority} required={required} onChange={(event) => updateForm(props.onChange, 'priority', event.target.value as Deployment['priority'])}>
    ...
    case 'status':
      return (
        <label className={wideClass}>
          Status{requiredMark}
          <select value={props.form.status} required={required} onChange={(event) => updateForm(props.onChange, 'status', event.target.value as Deployment['status'])}>
    ...
    case 'deployment_details':
      return (
        <div className="form-grid__wide form-grid">
          <FormSectionTitle title="Inzetgegevens" />
    ...
    case 'priority_teams':
      return (
        <div className="form-grid__wide form-grid">
          ...
          <label>
            Prioriteit
            <select value={props.form.priority} onChange={(event) => updateForm(props.onChange, 'priority', event.target.value as Deployment['priority'])}>
              <option value="low">Laag</option>
              <option value="normal">Normaal</option>
              <option value="high">Hoog</option>
              <option value="critical">Kritiek</option>
          ...
          <label>
            Status
            <select value={props.form.status} onChange={(event) => updateForm(props.onChange, 'status', event.target.value as Deployment['status'])}>
              <option value="draft">Concept</option>
              <option value="active">Actief</option>
              <option value="dispatching">Alarmeren</option>
              <option value="in_progress">In uitvoering</option>
    ...
    case 'custom_fields':
      return <DynamicDeploymentFields fields={props.customFields} values={props.form.customFields} onChange={props.onChange} />;
    default:
...
function deploymentFormLayout(
  layout: DeploymentFormLayoutItem[],
  customFields: ConfigurableFormField[],
): DeploymentFormLayoutItem[] {
  const defaults: DeploymentFormLayoutItem[] = [
    { key: 'section_deployment', label: 'Sectie: inzet', visible: true, width: 'full' },
  ...
  const merged = expandLegacyDeploymentLayout(layout, customFields)
...
function expandLegacyDeploymentLayout(
  layout: DeploymentFormLayoutItem[],
  customFields: ConfigurableFormField[],
): DeploymentFormLayoutItem[] {
  const replacements: Record<string, DeploymentFormLayoutItem[]> = {
    deployment_details: [
      { key: 'section_deployment', label: 'Sectie: inzet', visible: true, width: 'full' },
...
function deploymentLayoutItemRequired(item: DeploymentFormLayoutItem, enforceConfiguredRequired = true): boolean {
  return alwaysRequiredDeploymentModuleKeys.has(item.key) || (enforceConfiguredRequired && item.required === true);
}

function hasMissingRequiredFixedDeploymentInput(layout: DeploymentFormLayoutItem[], form: DeploymentFormState, enforceConfiguredRequired: boolean): boolean {
  return layout.some((item) => item.visible && fixedDeploymentInputModuleKeys.has(item.key) && deploymentLayoutItemRequired(item, enforceConfiguredRequired) && fixedDeploymentValue(item.key, form).trim() === '');
}

function fixedDeploymentValue(key: string, form: DeploymentFormState): string {
  switch (key) {
    case 'title':
      return form.title;
    case 'description':
...
function DynamicDeploymentFields(props: {
...
function DynamicDeploymentField(props: {
...
function updateCustomField(
  setForm: (updater: (current: DeploymentFormState) => DeploymentFormState) => void,
  key: string,
  value: unknown,
) {
  setForm((current) => ({
...
function LocationMap({ form, className }: { form: DeploymentFormState; className?: string }) {
...
export function deploymentPayload(
  ...
  const customFields = omitFixedDeploymentFields(form.customFields);
  ...
  return {
    ...
    ...deploymentStatusPayload(form.status, options.includeStatus === true),
    location_label: form.locationLabel.trim() === '' ? null : form.locationLabel,
...
export function formFromDeployment(deployment: Deployment): DeploymentFormState {
  const customFields = {
    ...(deployment.custom_fields ?? {}),
    reporter_name: (deployment.custom_fields ?? {}).reporter_name ?? deployment.reporter_name ?? '',
    reporter_phone: (deployment.custom_fields ?? {}).reporter_phone ?? deployment.reporter_phone ?? '',
    requesting_organization: (deployment.custom_fields ?? {}).requesting_organization ?? deployment.requesting_organization ?? '',
    requesting_unit: (deployment.custom_fields ?? {}).requesting_unit ?? deployment.requesting_unit ?? '',
    on_scene_contact_name: (deployment.custom_fields ?? {}).on_scene_contact_name ?? deployment.on_scene_contact_name ?? '',
    on_scene_contact_phone: (deployment.custom_fields ?? {}).on_scene_contact_phone ?? deployment.on_scene_contact_phone ?? '',
    on_scene_contact_role: (deployment.custom_fields ?? {}).on_scene_contact_role ?? deployment.on_scene_contact_role ?? '',
    required_resources: (deployment.custom_fields ?? {}).required_resources ?? deployment.required_resources ?? '',
  };

  return {
    title: deployment.title,
    description: deployment.description ?? '',
    reporterName: String((deployment.custom_fields ?? {}).reporter_name ?? deployment.reporter_name ?? ''),
    reporterPhone: String((deployment.custom_fields ?? {}).reporter_phone ?? deployment.reporter_phone ?? ''),
    requestingOrganization: deployment.requesting_organization ?? '',
    requestingUnit: deployment.requesting_unit ?? '',
    onSceneContactName: deployment.on_scene_contact_name ?? '',
    onSceneContactPhone: deployment.on_scene_contact_phone ?? '',
    onSceneContactRole: deployment.on_scene_contact_role ?? '',
    requiredResources: deployment.required_resources ?? '',
    priority: deployment.priority,
    status: deployment.status,
    locationLabel: deployment.location_label ?? '',
    latitude: deployment.latitude ?? '',
    longitude: deployment.longitude ?? '',
    coordinatorId: deployment.coordinator?.id ?? '',
    teamIds: deployment.teams?.length ? deployment.teams.map((team) => team.id) : deployment.team?.id ? [deployment.team.id] : [],
    customFields,
  };
}

function omitFixedDeploymentFields(fields: Record<string, unknown>): Record<string, unknown> {
...
function updateForm<K extends keyof DeploymentFormState>(
  setForm: (updater: (current: DeploymentFormState) => DeploymentFormState) => void,
  key: K,
