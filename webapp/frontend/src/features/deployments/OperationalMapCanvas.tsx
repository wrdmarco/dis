...
export interface OperationalMapDeploymentModel {
  deployment: { id: string; title: string };
  color: string;
...
export interface OperationalMapLayerVisibility {
  ...
  historicalDeployments: boolean;
  pilotHomes: boolean;
}

export interface OperationalMapLayerModels {
  commandCenters: Array<MapPoint & { id: string; name: string }>;
  historicalDeployments: Array<MapPoint & { id: string; reference: string; title: string }>;
    ...
    <div className="operational-map__canvas">
      <svg
        ...
        aria-label={models.length > 0
          ? `Kaart met inzetten, ${livePilotCount} actuele piloten en ${routeCount} navigatieroutes`
          : 'Kaart van Nederland zonder actieve inzetten'}
      >
        {tiles.map((tile) => (
          <image
        ...
        {showRoutes ? models.map((model) => model.liveLocations.map((location) => location.route === null ? null : (
          <PilotRoutePath
            key={`${model.deployment.id}-${location.userId}-route`}
            location={location}
            centerWorld={centerWorld}
            viewport={viewport}
            deploymentTitle={model.deployment.title}
          />
        ))) : null}
        {models.map((model) => model.deploymentPoint === null ? null : (
          <MapMarker
            key={`${model.deployment.id}-deployment`}
            point={model.deploymentPoint}
            centerWorld={centerWorld}
            viewport={viewport}
            color={model.color}
            label={model.deployment.title}
            type="deployment"
          />
        ))}
        {models.map((model) => model.liveLocations.map((location) => (
          <MapMarker
            key={`${model.deployment.id}-${location.userId}`}
            point={location}
            centerWorld={centerWorld}
            viewport={viewport}
        ...
        {layerVisibility.historicalDeployments ? layers.historicalDeployments.map((point) => (
          <HistoricalDeploymentMarker key={point.id} point={point} centerWorld={centerWorld} viewport={viewport} />
        )) : null}
      ...
      {models.length === 0 ? (
        <div className="operational-map__empty-state">
          <strong>Geen actieve inzetten</strong>
...
export function operationalMapAutoFitPoints({
  ...
  const operationalPoints = models.flatMap((model) => [
    ...(model.deploymentPoint ? [model.deploymentPoint] : []),
    ...model.liveLocations,
  ...
  return [
    ...
    ...(layerVisibility.historicalDeployments ? layers.historicalDeployments : []),
...
function PilotRoutePath({
  ...
  return (
    <path className="operational-map__route-line" d={path} fill="none" stroke={location.color} vectorEffect="non-scaling-stroke">
      <title>{`${location.name} naar ${deploymentTitle} - ${pilotRouteStatus(location.route)}`}</title>
    </path>
  );
}

function PilotRouteLegend({ models }: { models: OperationalMapDeploymentModel[] }) {
  const entries = models.flatMap((model) => model.liveLocations.map((location) => ({
    deploymentId: model.deployment.id,
    deploymentTitle: model.deployment.title,
    location,
  })));
  if (entries.length === 0) {
  ...
  return (
    <section className="operational-map__route-legend" aria-label="Pilootroutes">
      ...
      <ul>
        {entries.map(({ deploymentId, deploymentTitle, location }) => (
          <li key={`${deploymentId}-${location.userId}`}>
            ...
            <span>
              <b>{location.name}</b>
              <small>{`${deploymentTitle} · ${pilotOperationalDetail(location) ?? (location.route === null ? 'Route niet beschikbaar' : pilotRouteStatus(location.route))}`}</small>
            </span>
          </li>
        ))}
      </ul>
...
function MapMarker({ point, centerWorld, viewport, color, label, detail, type }: {
  ...
  const labelY = type === 'deployment' ? position.y - 14 : position.y + 5;

  return (
    <g className={`operational-map__marker operational-map__marker--${type}`}>
      <circle cx={position.x} cy={position.y} r={type === 'deployment' ? 11 : 7} fill={color} />
      <circle cx={position.x} cy={position.y} r={type === 'deployment' ? 17 : 12} stroke={color} />
      <title>{detail ? `${label} - ${detail}` : label}</title>
      <text x={labelX} y={labelY} textAnchor={labelAnchor}>
        <tspan x={labelX}>{shortMapLabel(label, type === 'deployment' ? 38 : 26)}</tspan>
...
function HistoricalDeploymentMarker({ point, centerWorld, viewport }: {
