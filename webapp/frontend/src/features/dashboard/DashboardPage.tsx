...
export function DashboardPage() {
  const deployments = useApiResource<Deployment[]>('/deployments?status=active');
  ...
  const criticalDeployments = activeDeployments.filter((deployment) => deployment.priority === 'critical').length;
  const highDeployments = activeDeployments.filter((deployment) => deployment.priority === 'high').length;
  const dispatchingDeployments = activeDeployments.filter((deployment) => deployment.status === 'dispatching').length;
    ...
    <div className="page-stack dashboard-page">
      ...
      <section className="dashboard-hero" aria-labelledby="dashboard-title">
        <div className="dashboard-hero__content">
          ...
          <h2 id="dashboard-title">Inzetten, mensen en middelen in een scanbaar overzicht</h2>
          <div className="dashboard-hero__signals" aria-label="Belangrijkste operationele signalen">
            <Signal label="Kritiek" value={criticalDeployments} tone={criticalDeployments > 0 ? 'bad' : 'good'} />
            <Signal label="Hoog" value={highDeployments} tone={highDeployments > 0 ? 'warn' : 'neutral'} />
            <Signal label="Alarmeren" value={dispatchingDeployments} tone={dispatchingDeployments > 0 ? 'warn' : 'neutral'} />
            <Signal label="Beschikbaar" value={`${availabilityRatio}%`} tone={availabilityRatio >= 60 ? 'good' : 'warn'} />
          </div>
        </div>
        <div className="dashboard-hero__actions">
          <Link className="primary-button" href="/inzetten">Inzetten openen <ArrowRight aria-hidden size={16} /></Link>
      ...
      <section className="kpi-grid dashboard-kpi-grid" aria-label="Operationele kengetallen">
        <Kpi icon={<RadioTower />} label="Actieve inzetten" value={activeDeployments.length} meta={`${criticalDeployments} kritiek`} tone={criticalDeployments > 0 ? 'red' : 'blue'} />
      ...
      <div className="dashboard-grid">
        <Panel title="Actieve inzetten">
          <ResourceState loading={deployments.loading} error={deployments.error} empty={(deployments.data?.length ?? 0) === 0}>
            <div className="dashboard-deployment-list">
              {activeDeployments.map((deployment) => (
                <Link className="dashboard-deployment" href={`/inzetten/${deployment.id}`} key={deployment.id}>
                  <span className={`dashboard-deployment__rail dashboard-deployment__rail--${deployment.priority}`} />
                  <span className="dashboard-deployment__main">
                    <strong>{deployment.reference}</strong>
                    <span>{deployment.title}</span>
                  </span>
                  <span className="dashboard-deployment__meta">
                    <StatusPill value={deployment.priority} tone={deployment.priority === 'critical' ? 'bad' : deployment.priority === 'high' ? 'warn' : 'neutral'} />
                    <StatusPill value={deployment.status} />
                  </span>
                  <span className="dashboard-deployment__location">
                    <MapPin aria-hidden size={15} />
                    {deployment.location_label || 'Locatie volgt'}
                  </span>
                </Link>
              ))}
            </div>
              ...
              {activeDispatches.slice(0, 8).map((dispatch) => (
                <article className="dashboard-dispatch" key={dispatch.id}>
                  <div className="dashboard-dispatch__body">
                    <strong>{dispatch.deployment?.reference ?? dispatch.deployment_id}</strong>
                    <span>{dispatch.message || dispatch.deployment?.title || 'Alarmering zonder bericht'}</span>
                  </div>
