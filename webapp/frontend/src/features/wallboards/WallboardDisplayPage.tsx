...
import { wallboardPlaylistPageLayers } from './wallboardPlaylistRendering';
...
export function WallboardDisplayPage() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (lastControlRuntimePlaylistSignatureRef.current === undefined) {
          lastControlRuntimePlaylistSignatureRef.current = wallboardRuntimePlaylistSignature(nextControl);
        }
  ...
  useEffect(() => {
    ...
    const pollControl = async () => {
      try {
        ...
        const nextRuntimePlaylistSignature = wallboardRuntimePlaylistSignature(nextControl);
        const runtimePlaylistChanged = lastControlRuntimePlaylistSignatureRef.current !== undefined
          && lastControlRuntimePlaylistSignatureRef.current !== nextRuntimePlaylistSignature;
        if (configVersionChanged || runtimePlaylistChanged) {
          await wallboardApi.get<null>('/wallboard/cache');
          if (cancelled) return;
          if (configVersionChanged) pendingConfigVersionRef.current = nextControl.config_version;
        ...
        const nextMaintenanceActive = wallboardMaintenanceNoticeIsActive(nextControl.maintenance);
        if (lastControlMaintenanceActiveRef.current === true && !nextMaintenanceActive) {
          needsStateRefresh = true;
        }
  ...
  return (
    ...
    >
      ...
      {maintenance !== null ? (
        <MaintenanceTakeover notice={maintenance} now={clock} />
...
export function WallboardPlaylistPageFrame({
  ...
  useEffect(() => {
    setVisual((current) => {
      if (current.currentPageId === page.id) return current;
  ...
  return (
    ...
    >
      ...
      >
        {layers.map((layer) => (
          <div
            className={`wallboard-display__page-card-pane${layer.role === 'outgoing'
              ? ' wallboard-display__page-card-pane--outgoing'
              : layer.role === 'current'
                ? ' wallboard-display__page-card-pane--incoming'
                : ''}`}
            style={layer.visible ? undefined : { display: 'none' }}
            hidden={!layer.visible}
            aria-hidden={layer.role === 'current' && active ? undefined : 'true'}
            data-wallboard-page-id={layer.page.id}
            data-wallboard-page-layer={layer.role}
            key={layer.page.id}
          >
...
function MaintenanceTakeover({
  ...
  notice: WallboardMaintenanceNotice;
  now: number;
}) {
  const estimate = wallboardMaintenanceEstimate(notice, now);
  ...
  return (
    ...
    >
      ...
      {estimate !== null ? (
        <div
          className="wallboard-display__maintenance-countdown"
          role="timer"
          aria-label={`Geschatte resterende updatetijd ${formatWallboardMaintenanceDuration(estimate.remainingSeconds)}`}
        >
          <svg viewBox="0 0 120 120" aria-hidden>
            <circle className="wallboard-display__maintenance-countdown-track" cx="60" cy="60" r="52" pathLength="100" />
            <circle
              className="wallboard-display__maintenance-countdown-progress"
              cx="60"
              cy="60"
              r="52"
              pathLength="100"
          ...
          <span>
            <strong>{estimate.remainingSeconds > 0
              ? formatWallboardMaintenanceDuration(estimate.remainingSeconds)
              : 'Nog even geduld'}</strong>
            <small>{estimate.remainingSeconds > 0 ? 'geschat resterend' : 'serverstatus controleren'}</small>
          </span>
        </div>
      ) : null}
  ...
  if (page.type === 'photo_carousel') {
    ...
    return (
      <WallboardPhotoCarousel
        media={media}
        running={running}
        transition={normalizeWallboardNewsItemTransition(page.options.item_transition)}
        transitionDurationMs={clampWallboardTransitionDurationMs(
          page.options.item_transition_duration_ms,
          DEFAULT_WALLBOARD_PHOTO_ITEM_TRANSITION_DURATION_MS,
        )}
        flipDirection={normalizeWallboardFlipDirection(page.options.item_flip_direction)}
...
function WallboardVideoPage({
  ...
  const embedUrl = wallboardVideoEmbedUrl(page.options.url, running);
  if (embedUrl === null) {
    return (
  ...
  return (
    <div className="wallboard-display__forecast">
      <section className={`wallboard-display__forecast-advice wallboard-display__forecast-advice--${forecast.overall_status}`} aria-label={`Vliegadvies: ${advice.label}`}>
        ...
        <div className="wallboard-display__forecast-advice-copy">
        ...
        <time className="wallboard-display__forecast-updated" dateTime={forecast.generated_at}>
          <Clock3 size={20} aria-hidden />
          <span>Laatst bijgewerkt</span>
          <strong>{formatWallboardForecastUpdateTime(forecast.generated_at)}</strong>
        </time>
      </section>
      {blocks.length > 0 ? (
...
export function formatWallboardForecastUpdateTime(value: string): string {
  ...
  return Number.isFinite(date.getTime()) ? WALLBOARD_CALENDAR_TIME_FORMATTER.format(date) : 'onbekend';
}
...
function WallboardNewsPage({
  ...
  const itemFlipDirection = normalizeWallboardFlipDirection(page.options.item_flip_direction);
  ...
  return (
    <div className="wallboard-display__news">
      <div
        ...
        style={{
          '--wallboard-news-item-duration': `${itemDurationSeconds}s`,
          '--wallboard-news-transition-duration': `${itemTransitionDurationMs}ms`,
        } as CSSProperties}
      >
        <WallboardNewsCardTransition
          item={activeItem}
...
function WallboardNewsCardTransition({
  item,
  transition,
  transitionDurationMs,
  flipDirection,
  ...
  const [visual, setVisual] = useState<{
    ...
    previous: WallboardNewsItem | null;
    sequence: number;
  }>(() => ({ current: item, previous: null, sequence: 0 }));
    ...
    >
      ...
      >
        {visual.previous === null ? null : (
          <div className="wallboard-display__news-card-pane wallboard-display__news-card-pane--outgoing" aria-hidden="true" key="outgoing">
            <NewsArticle item={visual.previous} transition="none" showProgress={false} />
          </div>
        )}
        <div className="wallboard-display__news-card-pane wallboard-display__news-card-pane--incoming" key={visual.current.id}>
          <NewsArticle item={visual.current} transition="none" />
        </div>
      </div>
    </div>
  );
...
function NewsArticle({
  ...
  return (
    <article className={`wallboard-display__news-article wallboard-display__news-article--${item.source} wallboard-display__news-article--transition-${transition} ${showImage ? 'wallboard-display__news-article--with-image' : 'wallboard-display__news-article--without-image'}`}>
      ...
      {showProgress ? <i className="wallboard-display__news-progress" aria-hidden /> : null}
    </article>
  );
}
...
export function normalizeWallboardMaintenanceNotice(value: unknown): WallboardMaintenanceNotice | null {
  ...
  const estimatedDurationSeconds = optionalPositiveInteger(value.estimated_duration_seconds);
  const estimatedCompletionAt = typeof value.estimated_completion_at === 'string'
    && Number.isFinite(Date.parse(value.estimated_completion_at))
    ? value.estimated_completion_at
    : null;
  const remainingSeconds = optionalNonNegativeInteger(value.remaining_seconds);
...
export function wallboardMaintenanceEstimate(
  notice: WallboardMaintenanceNotice,
  now: number = Date.now(),
): WallboardMaintenanceEstimate | null {
  if (!Number.isFinite(now) || notice.kind !== 'update') return null;
  const startedAt = Date.parse(notice.started_at);
  const estimatedCompletionAt = typeof notice.estimated_completion_at === 'string'
    ? Date.parse(notice.estimated_completion_at)
    : Number.NaN;
  const configuredDuration = optionalPositiveInteger(notice.estimated_duration_seconds);
  const derivedDuration = Number.isFinite(startedAt) && Number.isFinite(estimatedCompletionAt)
    ? Math.ceil((estimatedCompletionAt - startedAt) / 1000)
    : null;
  const totalSeconds = configuredDuration ?? derivedDuration;
  if (
    totalSeconds === null
    || totalSeconds < 1
    || !Number.isFinite(startedAt)
    || !Number.isFinite(estimatedCompletionAt)
    || estimatedCompletionAt <= startedAt
  ) return null;
...
export function controlFromState(state: WallboardState): WallboardControlState {
...
function normalizeRuntimePlaylistId(value: unknown): string | null {
  if (typeof value !== 'string') return null;
  const normalized = value.trim();
  return /^[0-9A-HJKMNP-TV-Z]{26}$/i.test(normalized) ? normalized : null;
}
