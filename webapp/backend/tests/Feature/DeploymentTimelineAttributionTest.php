...
final class DeploymentTimelineAttributionTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_kladblokregel_keeps_and_returns_the_recorded_author_name(): void
    {
        ...
        $this->grant($author, ['deployments.manage', 'deployments.view']);
        $this->grant($viewer, ['deployments.manage', 'deployments.view']);
        $deployment = $this->deployment($viewer, 'ATTR-NOTE-001');

        $this->asWebClient($author)
            ->patchJson('/api/deployments/'.$deployment->id.'/internal-notes', [
    ...
    public function test_operator_without_manage_permission_never_receives_kladblok_entries(): void
    {
        ...
        $deployment = $this->deployment($manager, 'ATTR-NOTE-HIDDEN-001');
        ...
        AuditLog::query()->create([
            ...
            'action' => 'deployments.internal_note_added',
    ...
    public function test_operator_app_receives_only_deployment_wide_and_own_timeline_rules_even_with_manage_permission(): void
    {
        ...
        $this->grant(
            $operator,
            ['deployments.assigned.view', 'deployments.manage'],
            operator: true,
            admin: false,
        );
        SystemSetting::query()->updateOrCreate(
            ['key' => 'deployment.timeline.app_visible_types'],
            [
        ...
        $deployment = $this->deployment($manager, 'ATTR-APP-SCOPE-001');
        $dispatch = DispatchRequest::query()->create([
            'deployment_id' => $deployment->id,
            'requested_by' => $manager->id,
            'requested_by_name' => $manager->name,
            ...
            'status' => 'sent',
            'priority' => 'normal',
            'message' => 'DEPLOYMENTBREDE-ALARMEERREGEL',
        ...
        $this->audit(
            'pilot_deployment_report.submitted_by_admin',
            'pilot-report',
            $manager,
            'EIGEN-INZETRAPPORT',
            ['deployment_id' => $deployment->id, 'user_id' => $operator->id],
            now()->subDays(2),
        );
        ...
        $this->audit(
            ...
            ['deployment_id' => $deployment->id, 'user_id' => $otherOperator->id],
        );
        $this->audit(
        ...
        $this->audit(
            'deployment.unknown_personal_action',
            $deployment,
            $otherOperator,
    ...
    public function test_timeline_separates_current_recipient_state_from_self_response_and_centralist_override(): void
    {
        ...
        $this->grant($manager, ['deployments.manage', 'deployments.view', 'deployments.dispatch.view']);
        $deployment = $this->deployment($manager, 'ATTR-LIVE-001');
        $statusHistory = $deployment->statusHistory()->create([
            'from_status' => 'draft',
            'to_status' => 'active',
        ...
        $this->assertSame('Inzetstatus gewijzigd: draft -> active door Mila Jansen', $items[$statusHistory->id]['description']);
    ...
    public function test_deployment_report_log_renders_the_same_actor_attribution(): void
    {
        $manager = $this->user('report-manager@example.test', 'Robin de Boer');
        $pilot = $this->user('report-pilot@example.test', 'Alex Piloot');
        $deployment = $this->deployment($manager, 'ATTR-REPORT-001', [
        ...
        $data = $this->app->make(DeploymentReportService::class)->data($deployment->fresh());
        $statusItem = collect($data['timeline'])->firstWhere('type', 'Inzetstatus');
        $dispatchItem = collect($data['timeline'])->firstWhere('type', 'Alarmering');

        $this->assertSame('Inzetstatus gewijzigd: draft -> active door Robin de Boer', $statusItem['description']);
        $this->assertSame('Alarmering aangemaakt door Robin de Boer', $dispatchItem['description']);

        $html = view('reports.deployment', $data)->render();
        $this->assertStringContainsString('Operationeel verloop', $html);
        $this->assertStringContainsString('Inzetstatus gewijzigd: draft -&gt; active door Robin de Boer', $html);
    ...
    private function deployment(User $creator, string $reference, array $overrides = []): Deployment
    {
        return Deployment::query()->create($overrides + [
            'reference' => $reference,
            'title' => 'Attributietestdeployment',
            'priority' => 'normal',
            'status' => 'active',
            'is_test' => false,
    ...
    {
        $token = $user->createToken('Deployment attribution test', ['*', 'client:web'], now()->addHour())->plainTextToken;
