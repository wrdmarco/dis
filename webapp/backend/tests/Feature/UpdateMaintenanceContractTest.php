...
final class UpdateMaintenanceContractTest extends TestCase
{
    ...
    public function test_deploy_mutates_the_resolved_persistent_environment_atomically(): void
    {
        $common = $this->read('scripts/lib/common.sh');
        $deploy = $this->read('scripts/deploy.sh');
        $mutation = $this->section(
            $common,
            'mutate_managed_env_value() (',
            "\n)\n\nset_managed_env_value()",
        );
        $dataPathCanonicalizer = $this->shellFunction($common, 'canonical_dis_data_path');
        $setter = $this->shellFunction($deploy, 'set_env_value');
        $migration = $this->shellFunction($deploy, 'migrate_env_key');

        self::assertStringContainsString(
            'resolved_data_path="$(/usr/bin/readlink -f -- "${DIS_DATA_PATH}" 2>/dev/null || true)"',
            $dataPathCanonicalizer,
        );
        self::assertStringContainsString(
        ...
        self::assertStringContainsString('sync -f "${temporary}"', $mutation);
        self::assertStringContainsString('sync -f "$(dirname "${resolved_env}")"', $mutation);
        self::assertStringNotContainsString('sed -i', $mutation);
        ...
        self::assertStringNotContainsString('sed -i', $setter.$migration);
        self::assertStringNotContainsString('>> "${ENV_FILE}"', $deploy);
    }

    public function test_deployment_location_enrichment_has_an_independent_low_priority_worker(): void
    {
        $service = $this->read('infrastructure/systemd/dis-deployment-enrichment.service');
        $queue = $this->read('infrastructure/systemd/dis-queue.service');
        $common = $this->read('scripts/lib/common.sh');
        $deploy = $this->read('scripts/deploy.sh');
        $restore = $this->read('scripts/restore.sh');
        ...
        self::assertStringContainsString('--queue=deployment-enrichment,incident-enrichment', $service);
        ...
        self::assertStringNotContainsString('deployment-enrichment', $queue);
        self::assertStringContainsString('dis-deployment-enrichment', $common);
        self::assertStringContainsString('infrastructure/systemd/dis-deployment-enrichment.service', $deploy);
        self::assertStringContainsString('dis-deployment-enrichment', $restore);
        self::assertStringContainsString('dis-deployment-enrichment', $selfHeal);
        self::assertStringContainsString('/etc/systemd/system/dis-deployment-enrichment.service', $uninstall);
    }

    public function test_first_domain_rename_update_uses_a_parent_bounded_legacy_enrichment_bridge(): void
    {
        $canonical = $this->read('infrastructure/systemd/dis-deployment-enrichment.service');
        $bridge = $this->read('infrastructure/systemd/dis-incident-enrichment-compat.service');
        $cutover = $this->read('scripts/deployment-enrichment-compat.sh');
        $deploy = $this->read('scripts/deploy.sh');
        $update = $this->read('scripts/update.sh');

        self::assertFileDoesNotExist(
            $this->repositoryRoot.'/infrastructure/systemd/dis-incident-enrichment.service',
        );
        self::assertStringContainsString('--queue=deployment-enrichment,incident-enrichment', $canonical);
        foreach ([
            '# DIS temporary legacy deployment-enrichment compatibility bridge',
            'Requires=dis-deployment-enrichment.service',
            'After=dis-deployment-enrichment.service',
            'BindsTo=dis-deployment-enrichment.service',
            'Type=oneshot',
            'ExecStart=/usr/bin/true',
            'RemainAfterExit=yes',
        ] as $contract) {
            self::assertStringContainsString($contract, $bridge);
        }
        self::assertStringNotContainsString('WantedBy=', $bridge);
        self::assertStringNotContainsString('artisan', $bridge);
        self::assertStringNotContainsString('queue:work', $bridge);
        ...
        self::assertStringContainsString(
            'Refusing to replace an unrecognized ${LEGACY_SERVICE}.service.',
            $cutover,
        );
        self::assertStringContainsString(
            'Refusing to shadow ${LEGACY_SERVICE}.service outside the managed unit path.',
            $cutover,
        );

        $processIdentity = $this->shellFunction($cutover, 'process_starttime');
        self::assertStringContainsString('/proc/{pid}/stat', $processIdentity);
        self::assertStringContainsString('fields[19]', $processIdentity);

        $scheduler = $this->shellFunction($cutover, 'schedule_compatibility_finalizer');
        self::assertStringContainsString(
            'parent_starttime="$(process_starttime "${parent_pid}")"',
            $scheduler,
        );
        self::assertStringContainsString('/usr/bin/systemd-run', $scheduler);
        self::assertStringContainsString(
            '--finalize-compat "${parent_pid}" "${parent_starttime}"',
            $scheduler,
        );

        $finalizer = $this->shellFunction($cutover, 'finalize_compatibility_bridge');
        self::assertStringContainsString(
        ...
        self::assertStringContainsString('canonical_unit_is_safe', $finalizer);
        self::assertStringContainsString('bridge_unit_is_safe', $finalizer);
        self::assertStringContainsString(
            'run_cmd rm -f -- "${LEGACY_WANTS_LINK}" "${LEGACY_UNIT}"',
            $finalizer,
        );
        self::assertStringNotContainsString('systemctl stop "${CANONICAL_SERVICE}"', $cutover);
        self::assertStringNotContainsString('systemctl stop dis-deployment-enrichment', $cutover);
        self::assertStringNotContainsString('rm -rf', $cutover);
        ...
        self::assertStringContainsString(
            'DIS_LEGACY_INCIDENT_ENRICHMENT_COMPAT_REQUIRED must be 0 or 1.',
            $deploy,
        );
        self::assertStringContainsString(
        ...
        $this->assertAppearsBefore(
            $deploy,
            'infrastructure/systemd/dis-deployment-enrichment.service',
            'bash "${SCRIPT_DIR}/deployment-enrichment-compat.sh" --retire',
        );
    }
