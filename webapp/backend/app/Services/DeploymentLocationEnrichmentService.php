...
final class DeploymentLocationEnrichmentService
{
    ...
    public function resolve(Deployment $deployment): bool
    {
        if (! (bool) config('dis.deployment_location.enabled', true) || $deployment->is_test) {
            return true;
        }

        $coordinates = $this->coordinateSnapshot($deployment);
        $provincePending = $deployment->province_resolved_at === null;
        $countryPending = $deployment->country_resolved_at === null;
        ...
        if (! $coordinates['valid']) {
            ...
            return $this->persistIfCoordinatesAreCurrent($deployment, $coordinates, $changes);
        }
        ...
        if ($provincePending) {
            ...
            if (! $this->persistIfCoordinatesAreCurrent($deployment, $coordinates, $changes)) {
                return false;
            }
        } elseif ($countryPending && $this->storedProvinceProvesNetherlands($deployment)) {
    ...
    private function fetchProvince(float $latitude, float $longitude): string
    {
        ...
        try {
            $response = Http::connectTimeout($this->timeout('connect_timeout_seconds', 2))
                ...
                ->withUserAgent('D.I.S Deployment Location Enrichment')
            ...
            throw new RuntimeException('Deployment province WFS transport failed.');
        }

        if (! $response->successful()) {
            throw new RuntimeException(sprintf('Deployment province WFS returned HTTP %d.', $response->status()));
        }

        return $this->boundedBody($response->body(), 'Deployment province WFS');
    }
    ...
    private function fetchCountry(float $latitude, float $longitude): string
    {
        ...
        try {
            ...
            throw new RuntimeException('Deployment country lookup transport failed.');
        }

        if (! $response->successful()) {
            throw new RuntimeException(sprintf('Deployment country lookup returned HTTP %d.', $response->status()));
        }

        return $this->boundedBody($response->body(), 'Deployment country lookup');
    }
    ...
    private function parseProvince(string $body): ?array
    {
        if (stripos($body, '<!DOCTYPE') !== false || stripos($body, '<!ENTITY') !== false) {
            throw new RuntimeException('Deployment province WFS returned unsafe XML.');
        }
        ...
        if (! $loaded || ! $document->documentElement instanceof DOMElement) {
            throw new RuntimeException('Deployment province WFS returned invalid XML.');
        }

        $xpath = new DOMXPath($document);
        $exception = $xpath->query('//*[local-name()="ExceptionReport" or local-name()="Exception"]');
        if ($exception === false || $exception->length > 0) {
            throw new RuntimeException('Deployment province WFS returned an exception document.');
        }

        if ($document->documentElement->localName !== 'FeatureCollection') {
            throw new RuntimeException('Deployment province WFS returned an unexpected XML document.');
        }

        $features = $xpath->query('//*[local-name()="member"]/*[local-name()="Provinciegebied"]');
        if ($features === false) {
            throw new RuntimeException('Deployment province WFS response could not be queried.');
        }
        ...
        if (! $feature instanceof DOMElement) {
            throw new RuntimeException('Deployment province WFS returned an invalid feature.');
        }
        ...
        if ($code === null || $name === null || (self::PROVINCES[$code] ?? null) !== $name) {
            throw new RuntimeException('Deployment province WFS returned a province outside the allowlist.');
        }
    ...
    private function parseCountry(string $body): ?array
    {
        try {
            ...
            throw new RuntimeException('Deployment country lookup returned invalid JSON.');
        }

        if (! is_array($payload) || ! array_is_list($payload)) {
            throw new RuntimeException('Deployment country lookup returned an invalid document.');
        }
        if ($payload === []) {
            return null;
        }
        if (count($payload) !== 1 || ! is_array($payload[0] ?? null) || ! is_array($payload[0]['attributes'] ?? null)) {
            throw new RuntimeException('Deployment country lookup returned an ambiguous result.');
        }
        ...
        if ($code === '' || $objectId !== $code) {
            throw new RuntimeException('Deployment country lookup returned an invalid country identifier.');
        }
     ...
     */
    private function coordinateSnapshot(Deployment $deployment): array
    {
        $latitude = $this->canonicalCoordinate($deployment->latitude);
        $longitude = $this->canonicalCoordinate($deployment->longitude);
     ...
     */
    private function persistIfCoordinatesAreCurrent(Deployment $deployment, array $coordinates, array $changes): bool
    {
        if ($changes === []) {
            return true;
        }
        ...
        $query = DB::table('deployments')
            ->where('id', (string) $deployment->getKey())
    ...
    private function storedProvinceProvesNetherlands(Deployment $deployment): bool
    {
        $code = (string) $deployment->province_code;

        return isset(self::PROVINCES[$code]) && self::PROVINCES[$code] === $deployment->province_name;
    }
    ...
    {
        $url = trim((string) config('dis.deployment_location.'.$key, $default));
        ...
        if (filter_var($url, FILTER_VALIDATE_URL) === false
            ...
            throw new RuntimeException('A configured deployment location enrichment URL is invalid.');
        }
        ...
        return [
            ...
            'on_headers' => static function (ResponseInterface $response): void {
                ...
                if ($encoding !== '' && $encoding !== 'identity') {
                    throw new RuntimeException('Deployment location enrichment returned encoded content.');
                }
                $length = trim($response->getHeaderLine('Content-Length'));
                if ($length !== '' && ctype_digit($length) && (int) $length > self::MAX_RESPONSE_BYTES) {
                    throw new RuntimeException('Deployment location enrichment response exceeded its size limit.');
                }
            },
    ...
    {
        return max(1, min(10, (int) config('dis.deployment_location.'.$key, $default)));
    }
}
