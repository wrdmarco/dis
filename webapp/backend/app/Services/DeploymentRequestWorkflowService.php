...
final class DeploymentRequestWorkflowService
{
    private const FORM_CONTRACT_ADVISORY_LOCK = 4920552417250622275;

    private ?DeploymentRequestWorkflowRevision $publishedCache = null;
    ...
    public function __construct(
        private readonly DeploymentRequestWorkflowRepository $repository,
    ...
    public function published(): DeploymentRequestWorkflowRevision
    {
        ...
        return $this->publishedCache = DB::transaction(fn (): DeploymentRequestWorkflowRevision => DeploymentRequestWorkflowRevision::query()->firstOrCreate(
            ['version' => 1],
            [
                'status' => 'published',
                'lock_version' => 1,
    ...
    public function adminEnvelope(): array
    {
        ...
        return [
            ...
            'history' => $this->repository->history()
                ->map(fn (DeploymentRequestWorkflowRevision $revision): array => $this->revisionPayload($revision))
                ->values()
                ->all(),
    ...
    public function publishDraft(int $expectedRevision, User $actor): array
    {
        DB::transaction(function () use ($expectedRevision, $actor): void {
            ...
            $nextVersion = ((int) (DeploymentRequestWorkflowRevision::query()->max('version') ?? 0)) + 1;
            $draft->forceFill([
                'version' => $nextVersion,
                'status' => 'published',
                'draft_marker' => null,
    ...
    /**
     * The current deployment form and published deployment-request workflow are one
    ...
    public function acquireFormContractMutationLock(): void
    {
        if (DB::transactionLevel() < 1) {
            throw new \LogicException('The deployment form contract lock requires an active transaction.');
        }
    ...
    public function validatePublishedFormContract(): DeploymentRequestWorkflowRevision
    {
        $published = $this->repository->published() ?? $this->published();
        $this->validateConfiguration(
            $published->configuration ?? [],
            allowHistoricalMirroredPhoneNumber: true,
        );
    ...
    {
        $source = DeploymentRequestWorkflowRevision::query()
            ->where('status', 'published')
    ...
    public function validateConfiguration(
        array $configuration,
        bool $allowHistoricalMirroredPhoneNumber = false,
    ): array {
        ...
        $bindings = $this->normalizeBindings(
            ...
            return [
                'triage' => [
                    ...
                    'reasons' => [$unknownRules !== []
                        ...
                        : 'Geen gepubliceerde prioriteitsregel past bij deze aanvraag.'],
                    'missing_fields' => [],
                ],
                'deployment_proposal' => null,
            ];
    ...
    public function normalizeAnswers(array $configuration, string $subjectType, array $answers, bool $patch = false): array
    {
        ...
        $fieldMap = collect($configuration['fields'] ?? [])
            ...
            ->keyBy('key');
        $targetDefinitions = collect($this->deploymentFieldCatalog())->keyBy('target')->all();
        $normalized = [];
        ...
        foreach ($answers as $key => $value) {
            ...
            if (! is_array($field) || ! $this->fieldApplies($field, $subjectType)) {
                throw ValidationException::withMessages(["answers.$key" => ['Dit veld hoort niet bij het gekozen aanvraagtype.']]);
            }
            $normalized[$key] = $value === null && $patch ? null : $this->normalizeAnswerValue($field, $value, "answers.$key");
            $this->assertBoundAnswerFitsCurrentDeploymentTarget(
                $configuration,
                $key,
                $normalized[$key],
                "answers.$key",
    ...
    /**
     * Revalidates frozen bound values against the deployment form that is
     * current at deployment-preparation time, so removed or narrowed targets never vanish
     * silently from the resulting deployment.
     *
     * @param  array<string, mixed>  $configuration
     * @param  array<string, mixed>  $answers
     */
    ...
    /** @return array<string, mixed> */
    public function revisionPayload(DeploymentRequestWorkflowRevision $revision): array
    {
        return [
            'id' => $revision->id,
    ...
    {
        $deployment->loadMissing('deploymentRequest.workflowRevision');
        $deploymentRequest = $deployment->deploymentRequest;
        if ($deploymentRequest === null) {
            return [];
        }
        $frozenConfiguration = $deploymentRequest->workflowRevision->configuration ?? [];
    ...
    {
        return [
            'deployment_fields' => $this->deploymentFieldCatalog(),
            'teams' => Team::query()
    ...
    public function defaultConfiguration(): array
    {
        $configuration = [
            ...
            'priority_rules' => [
                ...
                ['id' => 'baseline_complete', 'label' => 'Volledige standaardaanvraag', 'subject_types' => self::SUBJECT_TYPES, 'match' => 'all', 'conditions' => [], 'priority' => 'low', 'explanation' => 'De verplichte gegevens zijn compleet en er zijn geen hogere risicoregels geactiveerd.', 'deployment_profile_id' => 'standard_response'],
            ],
            'deployment_profiles' => [
        ...
        return $this->synchronizeInitialDeploymentFormFields($configuration);
    }

    /**
     * Keeps first-time workflow initialization compatible with the deployment
     ...
     * a matching common deployment-request field.
     *
     * @param  array<string, mixed>  $configuration
     * @return array<string, mixed>
     */
    private function synchronizeInitialDeploymentFormFields(array $configuration): array
    {
        $deploymentFields = collect($this->deploymentFormService->fields())
            ->filter(fn (array $field): bool => ($field['type'] ?? null) !== 'section'
                && ($field['key'] ?? null) !== 'required_resources')
            ->values();
        $deploymentFieldsByTarget = $deploymentFields->keyBy(
        ...
        foreach ($configuration['bindings'] as $bindingIndex => $binding) {
            ...
            $deploymentField = $deploymentFieldsByTarget->get($canonicalTarget);

            if (! is_int($fieldIndex) || ! is_array($deploymentField)) {
        ...
        foreach ($deploymentFields as $deploymentField) {
            if (($deploymentField['visible'] ?? true) !== true
                || ($deploymentField['required'] ?? false) !== true
            ) {
                continue;
            }
            $target = 'custom_fields.'.$deploymentField['key'];
            if (in_array(self::canonicalBindingTarget($target), $boundTargets, true)) {
                continue;
            }
            $hash = substr(hash('sha256', (string) $deploymentField['key']), 0, 8);
            $base = substr((string) $deploymentField['key'], 0, 36);
            ...
            $configuration['fields'][] = $this->alignInitialWorkflowField([
                'key' => $fieldKey,
                'label' => (string) ($deploymentField['label'] ?? $deploymentField['key']),
                'type' => 'text',
                'scope' => 'common',
                'required' => true,
     ...
     * @param  array<string, mixed>  $deploymentField
     * @return array<string, mixed>
     */
    private function alignInitialWorkflowField(array $workflowField, array $deploymentField): array
    {
        ...
        return [
            ...
            'required' => ($workflowField['required'] ?? false) === true
                || ($deploymentField['required'] ?? false) === true,
            'help_text' => $deploymentType === 'flight_time'
    ...
    private function ensureDraft(DeploymentRequestWorkflowRevision $published): DeploymentRequestWorkflowRevision
    {
        return DeploymentRequestWorkflowRevision::query()->firstOrCreate(
            ['draft_marker' => 'active'],
            [
                'status' => 'draft',
                'lock_version' => 1,
    ...
    private function normalizeSubjectTypes(mixed $subjectTypes): array
    {
        ...
        foreach ($subjectTypes as $index => $subjectType) {
            if (! is_array($subjectType) || array_diff(array_keys($subjectType), ['key', 'label']) !== []) {
                throw ValidationException::withMessages(["configuration.subject_types.$index" => ['Aanvraagtype is ongeldig.']]);
            }
    ...
    private function normalizeBindings(
        mixed $bindings,
        array $fieldMap,
        bool $allowHistoricalMirroredPhoneNumber = false,
    ): array {
        ...
        $targetMap = collect($this->catalogs()['deployment_fields'])->keyBy('target');
        $normalized = [];
        foreach ($bindings as $index => $binding) {
            ...
            if (! is_string($binding['field_key'] ?? null) || ! is_string($binding['target'] ?? null)) {
                throw ValidationException::withMessages(["configuration.bindings.$index" => ['Veld of inzetdoel is niet toegestaan.']]);
            }
            ...
            if (! $this->bindingTypesCompatible($fieldMap[$fieldKey], $targetDefinition)
                && ! ($allowHistoricalMirroredPhoneNumber
                    && $this->historicalMirroredPhoneNumberIsCompatible(
                        $fieldMap[$fieldKey],
                        $targetDefinition,
                        $target,
                    ))) {
                throw ValidationException::withMessages(["configuration.bindings.$index" => ['Het uitvraagveld en inzetveld hebben geen compatibele typen.']]);
            }
            if (isset($normalized[$fieldKey])) {
                throw ValidationException::withMessages(["configuration.bindings.$index.field_key" => ['Een uitvraagveld kan maar één inzetveld vullen.']]);
            }
                ...
                for ($right = $left + 1; $right < count($bindingsForTarget); $right++) {
                    ...
                    if ($leftScope === 'common' || $rightScope === 'common' || $leftScope === $rightScope) {
                        throw ValidationException::withMessages([
                            'configuration.bindings' => ["Inzetdoel '$target' wordt door gelijktijdig actieve velden dubbel gevuld."],
                        ]);
                    }
                }
            }
        ...
        foreach ($rules as $index => $rule) {
            ...
            if (is_string($profileId)) {
                ...
                if (! in_array($priority, $profile['priorities'], true)
                    ...
                    throw ValidationException::withMessages(["configuration.priority_rules.$index.deployment_profile_id" => ['Het inzetprofiel dekt niet alle aanvraagtypen en de prioriteit van deze regel.']]);
                }
            }
            $normalized[$id] = [
                'id' => $id,
        ...
        foreach ($conditions as $index => $condition) {
            ...
            foreach ($subjectTypes as $subjectType) {
                if (! $this->fieldApplies($field, $subjectType)) {
                    throw ValidationException::withMessages(["configuration.priority_rules.$ruleIndex.conditions.$index.field_key" => ['Het veld is niet beschikbaar voor alle aanvraagtypen van deze regel.']]);
                }
            }
    ...
    private function historicalMirroredPhoneNumberIsCompatible(
        array $sourceField,
        array $targetDefinition,
        string $target,
    ): bool {
        return self::canonicalBindingTarget($target) === 'on_scene_contact_phone'
            && ($sourceField['type'] ?? null) === 'number'
            && in_array($targetDefinition['type'] ?? null, ['phone', 'text'], true);
    }

    /**
     * @param  array<string, mixed>  $configuration
     */
    private function assertBoundAnswerFitsCurrentDeploymentTarget(
        array $configuration,
        string $fieldKey,
        mixed $value,
        string $path,
        ...
        if (! is_array($definition)) {
            throw ValidationException::withMessages([
                $path => ["Het gekoppelde inzetveld '$target' is niet meer beschikbaar."],
            ]);
        }
        ...
        if (($definition['type'] ?? null) === 'number' && is_int($value)) {
            ...
            if (($minimumValue !== null && $value < $minimumValue)
                ...
                throw ValidationException::withMessages([
                    $path => ["Gebruik een waarde van $minimumValue tot en met $maximumValue voor het gekoppelde inzetveld."],
                ]);
            }
        }
        if (in_array($definition['type'] ?? null, ['select', 'radio'], true)
            ...
            throw ValidationException::withMessages([
                $path => ['Deze waarde is niet beschikbaar in het gekoppelde inzetveld.'],
            ]);
        }
     ...
     * @param  array<string, array<string, mixed>>  $fieldMap
     */
    private function assertRequiredDeploymentBindings(array $bindings, array $fieldMap): void
    {
        ...
        foreach (self::SUBJECT_TYPES as $subjectType) {
            foreach ($requiredTargets as $target) {
                ...
                if (! $covered) {
                    throw ValidationException::withMessages([
                        'configuration.bindings' => ["Aanvraagtype '$subjectType' mist een verplicht, vereist en waar nodig operator-zichtbaar veld voor inzetdoel '$target'."],
                    ]);
                }
            }
        }
