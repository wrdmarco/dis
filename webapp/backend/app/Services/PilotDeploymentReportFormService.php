...
final class PilotDeploymentReportFormService
{
     ...
     */
    public function fields(?User $user = null, bool $operatorOnly = false, ?Deployment $deployment = null): array
    {
        ...
        $currentReport = $user !== null && $deployment !== null
            ? PilotDeploymentReport::query()
     ...
     */
    public function validationRules(?User $user = null, ?Deployment $deployment = null): array
    {
        $rules = [
            'custom_fields' => ['nullable', 'array'],
        ];
        $currentDroneSelections = $this->currentDroneSelections($user, $deployment);
    ...
    private function withResolvedOptions(
        array $field,
        ?User $user,
        ?PilotDeploymentReport $currentReport = null,
    ): array {
     ...
     */
    private function currentDroneOption(array $field, ?PilotDeploymentReport $report): ?array
    {
    ...
    private function currentDroneSelections(?User $user, ?Deployment $deployment): array
    {
        if ($user === null || $deployment === null) {
            return [];
        }

        $report = PilotDeploymentReport::query()
            ->where('deployment_id', $deployment->id)
            ->where('user_id', $user->id)
            ->first(['custom_fields']);
        $customFields = is_array($report?->custom_fields) ? $report->custom_fields : [];
