...
final class DeploymentReportService
{
    private const CANONICAL_REPORT_DIRECTORY = 'deployment-reports';
    ...
    /**
     * @param  Collection<int, PilotDeploymentReport>  $pilotReports
     * @return Collection<int, array<string, mixed>>
     */
    private function pilotReportRows(Collection $pilotReports): Collection
    {
        return $pilotReports->map(function (PilotDeploymentReport $report): array {
    ...
    public function pdf(Deployment $deployment, bool $preserveExistingMaps = false): string
    {
        ...
        $data = $this->data($deployment, $preserveExistingMaps);
        try {
    ...
    {
        try {
            return view('reports.deployment', $data)->render();
        } catch (Throwable $exception) {
    ...
    private function isMissingCompiledReportView(Throwable $exception): bool
    {
        ...
        return str_contains($message, 'File does not exist at path')
            ...
            && str_contains($message, 'reports/deployment.blade.php');
    }
    ...
    private function compileReportView(): void
    {
        ...
        $viewPath = resource_path('views/reports/deployment.blade.php');
    ...
    public function ensureStored(Deployment $deployment): ?string
    {
        if (! in_array($deployment->status, ['resolved', 'cancelled'], true)) {
            return null;
        }

        $storedPath = $this->validatedStoredReportPath($deployment);
        $hasStoredReport = $storedPath !== null
            && $this->storedReportExists($deployment, $storedPath);

        if ($hasStoredReport && $deployment->report_finalized_at !== null) {
            return $this->canonicalizeStoredReportPath($deployment, $storedPath);
        }

        if ($hasStoredReport && ! $this->reportCanBeFinalized($deployment)) {
            return $this->canonicalizeStoredReportPath($deployment, $storedPath);
        }

        return $this->storePdf($deployment, $hasStoredReport);
    }

    public function refreshStored(Deployment $deployment, bool $preserveExistingMaps = false): ?string
    {
        if (! in_array($deployment->status, ['resolved', 'cancelled'], true)) {
            return null;
        }
        ...
        if ($storedPath !== null
            && $this->storedReportExists($deployment, $storedPath)
            && $deployment->report_finalized_at !== null) {
            return $this->canonicalizeStoredReportPath($deployment, $storedPath);
        }

        return $this->storePdf($deployment, $preserveExistingMaps);
    }

    public function storedPdfPath(Deployment $deployment): ?string
    {
        $storedPath = $this->validatedStoredReportPath($deployment);
        if ($storedPath === null) {
            return null;
        ...
        try {
            $path = $this->absoluteReportPath($storedPath);
            if (! is_file($path)) {
                return null;
            }
            ...
            if ($this->reportDirectoryForPath($deployment, $storedPath) === self::CANONICAL_REPORT_DIRECTORY) {
                $this->deleteLegacyReportDirectory($deployment);
            }
    ...
    private function storedReportExists(Deployment $deployment, string $path): bool
    {
        if ($this->reportDirectoryForPath($deployment, $path) === null) {
            return false;
        }
    ...
    private function validatedStoredReportPath(Deployment $deployment): ?string
    {
        $path = $deployment->report_pdf_path;
        if (! is_string($path) || $path === '') {
            return null;
        }

        return $this->reportDirectoryForPath($deployment, $path) !== null
            ? $path
            : null;
    }

    private function reportDirectoryForPath(Deployment $deployment, string $path): ?string
    {
        $deploymentId = (string) $deployment->getKey();
        if (! Str::isUlid($deploymentId)
            || preg_match('/[\\\\\x00-\x1F\x7F]/', $path) !== 0) {
            return null;
        }

        foreach ([self::CANONICAL_REPORT_DIRECTORY, self::LEGACY_REPORT_DIRECTORY] as $directory) {
            $prefix = $directory.'/'.$deploymentId.'/';
    ...
    private function canonicalizeStoredReportPath(Deployment $deployment, string $storedPath): ?string
    {
        if ($this->reportDirectoryForPath($deployment, $storedPath) === self::CANONICAL_REPORT_DIRECTORY) {
            $this->deleteLegacyReportDirectory($deployment);
        ...
        return $this->migrateLegacyStoredReport($deployment, $storedPath);
    }

    private function migrateLegacyStoredReport(Deployment $deployment, string $legacyPath): ?string
    {
        if ($this->reportDirectoryForPath($deployment, $legacyPath) !== self::LEGACY_REPORT_DIRECTORY) {
            return null;
        }

        try {
            $legacyAbsolutePath = $this->absoluteReportPath($legacyPath);
            if (! is_file($legacyAbsolutePath) || ! is_readable($legacyAbsolutePath)) {
                return null;
            }

            $canonicalPath = $this->reportPath($deployment);
            $canonicalAbsolutePath = $this->absoluteReportPath($canonicalPath);
            $this->ensureWritableDirectory(dirname($canonicalAbsolutePath));
            if (! File::copy($legacyAbsolutePath, $canonicalAbsolutePath)) {
                throw new \RuntimeException('Het bestaande inzetrapport kon niet veilig worden gemigreerd.');
            }
            $legacyHash = hash_file('sha256', $legacyAbsolutePath);
            $canonicalHash = hash_file('sha256', $canonicalAbsolutePath);
            if (! is_string($legacyHash)
                || ! is_string($canonicalHash)
                || ! hash_equals($legacyHash, $canonicalHash)) {
                throw new \RuntimeException('De integriteitscontrole van het gemigreerde inzetrapport is mislukt.');
            }
            @chmod($canonicalAbsolutePath, 0660);
            ...
            $this->safeReport($exception);
            $deployment->forceFill([
                'report_generation_error' => 'Report migration failed. See secured server logs.',
            ])->save();
    ...
    private function storePdf(Deployment $deployment, bool $preserveExistingMaps = false): ?string
    {
        try {
            $path = $this->reportPath($deployment);
            $isFinal = $this->reportCanBeFinalized($deployment);
            $absolutePath = $this->absoluteReportPath($path);
            $this->ensureWritableDirectory(dirname($absolutePath));
            if (File::put($absolutePath, $this->pdf($deployment, $preserveExistingMaps)) === false) {
                throw new \RuntimeException('Inzetrapport kon niet worden opgeslagen op '.$absolutePath.'.');
            }
    ...
    private function deleteLegacyReportDirectory(Deployment $deployment): void
    {
        $deploymentId = (string) $deployment->getKey();
        if (! Str::isUlid($deploymentId)) {
            return;
        ...
        $legacyDirectory = self::LEGACY_REPORT_DIRECTORY.'/'.$deploymentId;
        try {
            Storage::disk('local')->deleteDirectory($legacyDirectory);
        } catch (Throwable $exception) {
            // The canonical PDF is already durable and referenced. A failed
            // legacy cleanup must not roll that successful generation back.
            $this->safeReport($exception);
        }
    }

    public function deleteStoredReportDirectories(string $deploymentId): void
    {
        if (! Str::isUlid($deploymentId)) {
            throw new \InvalidArgumentException('Ongeldige inzetidentiteit voor rapportopslag.');
        }
        ...
        foreach ([self::CANONICAL_REPORT_DIRECTORY, self::LEGACY_REPORT_DIRECTORY] as $directory) {
            try {
                $disk->deleteDirectory($directory.'/'.$deploymentId);
            } catch (Throwable $exception) {
        ...
        if ($firstFailure !== null) {
            throw new \RuntimeException(
                'Niet alle opgeslagen inzetrapporten konden veilig worden verwijderd.',
                0,
                $firstFailure,
            );
        }
    ...
    public function reportCanBeFinalized(Deployment $deployment): bool
    {
        return $this->missingPilotReportCount($deployment) === 0
            && $this->unfinalizedPilotReportCount($deployment) === 0;
    }

    public function missingPilotReportCount(Deployment $deployment): int
    {
        $acceptedUserIds = $this->acceptedPilotReportUserIds($deployment);
        if ($acceptedUserIds->isEmpty()) {
            return 0;
        }

        $submittedUserIds = $deployment->pilotReports()
            ->where('status', 'submitted')
    ...
    public function unfinalizedPilotReportCount(Deployment $deployment): int
    {
        $acceptedUserIds = $this->acceptedPilotReportUserIds($deployment);
        if ($acceptedUserIds->isEmpty()) {
            return 0;
        ...
        return $deployment->pilotReports()
            ->where('status', 'submitted')
            ->whereIn('user_id', $acceptedUserIds->all())
            ->get()
            ->filter(fn (PilotDeploymentReport $report): bool => ! $report->isFinalized())
            ->count();
    }
     ...
     */
    private function acceptedPilotReportUserIds(Deployment $deployment): Collection
    {
        $dispatches = $deployment->dispatchRequests()
            ->with('recipients')
     ...
     * @return Collection<int, array<string, mixed>>
     */
    private function timeline(Deployment $deployment, Collection $dispatches): Collection
    {
     ...
     * @return array<string, mixed>
     */
    private function mapData(Deployment $deployment, ?array $droneFlightContext, bool $preserveExistingMaps = false): array
    {
                ...
                foreach ($this->mapTileUrls($zoom, $tileY, $wrappedTileX) as $index => $url) {
                    ...
                    try {
                        $response = Http::timeout(1)
                            ->withHeaders([
                                'User-Agent' => 'DIS Deployment Report/1.0 (https://dis.wrdmarco.nl)',
                            ])
                            ->get($url);
                    } catch (Throwable) {
                        continue;
    ...
    private function mapSnapshotPng(int $width, int $height, array $tiles): ?string
    {
        ...
        imagestring($canvas, 2, 18, $height - 24, 'Inzetlocatie', $text);
    ...
    private function safeReport(Throwable $exception): void
    {
        try {
            ...
            // A logging backend failure must not block deployment report generation.
        }
    }
