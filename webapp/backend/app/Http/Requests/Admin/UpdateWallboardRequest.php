...
final class UpdateWallboardRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'active_deployment_playlist_id' => [
                'sometimes',
                'nullable',
                'ulid',
                Rule::exists('wallboard_playlists', 'id')
                    ->where('purpose', WallboardPlaylist::PURPOSE_ALARM)
            ...
            'configuration' => ['sometimes', 'required', 'array:theme,refresh_seconds,rotation_enabled,page_fade_enabled,page_transition,page_transition_duration_ms,page_flip_direction,pages,focus,deployment_override,ticker,map'],
            ...
            'configuration.pages.*.options' => ['sometimes', 'array:body,content,quotes,show_test_deployments,sources,custom_sources,max_items,item_duration_seconds,item_transition,item_transition_duration_ms,item_flip_direction,url,video_duration_seconds,media_playlist_id,location_mode,location_label,latitude,longitude,visible_blocks,visible_metrics,metric_visualizations,radar_kind'],
            ...
            'configuration.pages.*.options.show_test_deployments' => ['sometimes', 'boolean'],
            ...
            'configuration.deployment_override' => ['sometimes', 'array:enabled,page_id'],
            'configuration.deployment_override.enabled' => ['sometimes', 'boolean'],
            'configuration.deployment_override.page_id' => ['sometimes', 'nullable', 'string', 'max:64', 'regex:/^[A-Za-z0-9][A-Za-z0-9_-]*$/'],
            ...
            'configuration.map' => ['sometimes', 'array:show_active_deployments,show_test_deployments,show_live_locations,show_routes,show_command_centers,show_historical_deployments,show_summary,show_deployment_list,show_route_legend,auto_fit'],
            'configuration.map.show_active_deployments' => ['sometimes', 'boolean'],
            'configuration.map.show_test_deployments' => ['sometimes', 'boolean'],
            ...
            'configuration.map.show_historical_deployments' => ['sometimes', 'boolean'],
            'configuration.map.show_summary' => ['sometimes', 'boolean'],
            'configuration.map.show_deployment_list' => ['sometimes', 'boolean'],
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            if (($this->has('configuration') || $this->has('layout') || $this->has('display_profile') || $this->has('active_deployment_playlist_id'))
    ...
    private function validatePageOptions(Validator $validator): void
    {
        foreach ((array) $this->input('configuration.pages', []) as $index => $page) {
            ...
            if ($type === 'kpi') {
                try {
                    WallboardConfiguration::normalizeKpiOptions(
                        $options,
                        "configuration.pages.{$index}.options",
                    );
                } catch (ValidationException $exception) {
