...
<head>
    ...
    <style>
        ...
        .aeret-link-card { padding: 10px 12px; border: 1px solid #bfdbfe; border-radius: 9px; background: #eff6ff; }
        ...
        .aeret-snapshot { display: block; width: 100%; height: 360px; margin-bottom: 8px; border: 1px solid #cbd5e1; border-radius: 9px; object-fit: cover; object-position: center center; }
...
@endphp
<div class="footer">D.I.S inzetrapport - gegenereerd {{ $formatDate($generatedAt) }}</div>

<header class="header">
    <div class="header__eyebrow">Nationaal Droneteam - Inzetrapport</div>
    <h1>{{ $deployment->reference }}</h1>
    <div class="header__meta">{{ $deployment->title }} - {{ ucfirst($deployment->status) }}</div>
</header>
...
<section class="section">
    <table class="grid">
        ...
        <tr>
            ...
            <td class="metric"><span>Geopend</span><strong>{{ $formatDate($deployment->opened_at) }}</strong></td>
            <td class="metric"><span>Gesloten</span><strong>{{ $formatDate($deployment->closed_at) }}</strong></td>
        </tr>
    </table>
</section>
...
<section class="section">
    <h2>Inzetgegevens</h2>
    <table class="details">
        <tr><th>Omschrijving</th><td>{{ $deployment->description ?: '-' }}</td></tr>
        <tr><th>Melder</th><td>{{ $deployment->reporter_name ?: '-' }}{{ $deployment->reporter_phone ? ' - '.$deployment->reporter_phone : '' }}</td></tr>
        <tr><th>Aanvragende organisatie</th><td>{{ $deployment->requesting_organization ?: '-' }}{{ $deployment->requesting_unit ? ' - '.$deployment->requesting_unit : '' }}</td></tr>
        <tr><th>Contact ter plaatse</th><td>{{ $deployment->on_scene_contact_name ?: '-' }}{{ $deployment->on_scene_contact_role ? ' - '.$deployment->on_scene_contact_role : '' }}{{ $deployment->on_scene_contact_phone ? ' - '.$deployment->on_scene_contact_phone : '' }}</td></tr>
        <tr><th>Prioriteit</th><td>{{ ucfirst($deployment->priority) }}</td></tr>
        <tr><th>Opkomstlocatie</th><td>{{ $deployment->location_label ?: '-' }}</td></tr>
        <tr><th>Team</th><td>{{ $deployment->team ? $deployment->team->code.' - '.$deployment->team->name : '-' }}</td></tr>
        <tr><th>Coordinator</th><td>{{ $deployment->coordinator?->name ?? $deployment->coordinator_name ?? '-' }}</td></tr>
        <tr><th>Aangemaakt door</th><td>{{ $deployment->creator?->name ?? $deployment->created_by_name ?? '-' }}</td></tr>
        <tr><th>Benodigde middelen</th><td>{{ $deployment->required_resources ?: '-' }}</td></tr>
        @foreach (($deployment->custom_fields ?? []) as $customKey => $customValue)
...
<section class="section">
    <h2>Inzetkaart satelliet</h2>
    @if ($map['available'])
        <div class="map-layout">
            <div class="map-card">
                @if (! empty($map['snapshot_data_uri']))
                    <img class="map-snapshot" src="{{ $map['snapshot_data_uri'] }}" alt="Inzetkaart satelliet snapshot">
                @else
            ...
            <div class="map-detail-box">
                <strong>{{ $deployment->location_label ?: 'GPS locatie' }}</strong>
                <span>Latitude: {{ $map['latitude_label'] }}</span>
                <span>Longitude: {{ $map['longitude_label'] }}</span>
                @if (! empty($map['openstreetmap_url']))
                    <div class="map-url">Inzetkaart OSM: {{ $map['openstreetmap_url'] }}</div>
                @endif
            </div>
        </div>
    @else
        <p class="muted">Geen GPS-locatie beschikbaar voor deze inzet.</p>
    @endif
</section>
...
<section class="section">
                    ...
                    @if ($flightMap || $aeretUrl)
                        ...
                        @else
                            <div class="aeret-link-card">
                                ...
                                <span>Snapshot kon niet worden vastgelegd. Gebruik deze link voor de actuele kaart op dezelfde inzetlocatie.</span>
                            </div>
                            <p class="map-url">Aeret kaart: {{ $aeretUrl ?? '-' }}</p>
                        @endif
                    @else
    ...
    @else
        <p class="muted">Geen dronevluchtinformatie opgeslagen voor deze inzet.</p>
    @endif
</section>
