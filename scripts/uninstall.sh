...
for service in dis-media dis-queue dis-scheduler dis-websocket dis-osrm dis-incident-enrichment dis-knmi \
...
if [ "${PURGE_PACKAGES}" = "1" ]; then
  ...
  run_cmd apt-get purge -y \
    composer ffmpeg nginx postgresql postgresql-client redis-server redis-tools cifs-utils smbclient libeccodes-tools \
