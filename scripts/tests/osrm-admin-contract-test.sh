...
assert_contains "${worker}" 'Bestaande geverifieerde OSRM-container blijft ongewijzigd tijdens de kaartupdate.'
...
assert_contains "${common}" 'verify_existing_osrm_admin_runtime_library_for_upgrade'
assert_contains "${common}" '$'\''common.sh\ndis-osrm.service\nosrm.sh\nsecure-path.py'\'''
assert_contains "${common}" '"${app_root}/scripts/osrm-containers.conf"'
assert_contains "${common}" '"${staging}/containers.conf"'
...
assert_contains "${osrm}" 'apt-mark hold podman'
assert_contains "${osrm}" 'apt-mark hold osmium-tool'
assert_contains "${osrm}" 'ghcr.io/project-osrm/osrm-backend@${OSRM_CONTAINER_IMAGE_DIGEST}'
assert_contains "${osrm}" 'OSRM_CONTAINER_IMAGE_DIGEST="sha256:'
assert_contains "${osrm}" 'test("^(sha256:)?[a-f0-9]{64}$")'
assert_contains "${osrm}" 'image_id="${image_id#sha256:}"'
assert_absent "${osrm}" 'ghcr.io/project-osrm/osrm-backend:latest'
assert_contains "${osrm}" '--pull=never'
assert_contains "${osrm}" 'The pinned OSRM container could not start for its offline profile integrity check.'
...
assert_contains "${osrm}" '"${OSRM_PODMAN_PATH}" "${OSRM_PODMAN_GLOBAL_ARGS[@]}"'
assert_contains "${osrm}" 'run_podman() ('
assert_contains "${osrm}" 'export CONTAINERS_CONF="${OSRM_PODMAN_CONTAINERS_CONF}"'
assert_contains "${osrm}" '/usr/bin/env "CONTAINERS_CONF=${OSRM_PODMAN_CONTAINERS_CONF}"'
assert_contains 'scripts/osrm-containers.conf' 'default_sysctls = []'
assert_contains "${osrm}" 'exec {inherited_lock_fd}>&-'
assert_contains "${osrm}" 'exec 9>&-'
assert_contains "${osrm}" 'Podman requires /dev/fuse for its LXC-safe overlay store.'
assert_contains "${osrm}" '--cap-drop=all'
...
assert_contains "${worker}" 'shopt -s lastpipe'
assert_absent "${worker}" 'mkfifo -m 0600'
assert_contains 'scripts/uninstall.sh' 'for held_package in podman fuse-overlayfs osrm-backend osmium-tool; do'
assert_contains 'scripts/uninstall.sh' 'run_cmd apt-mark unhold "${held_package}"'
assert_contains 'scripts/uninstall.sh' 'for osrm_package in fuse-overlayfs osrm-backend osmium-tool; do'
assert_contains 'scripts/uninstall.sh' 'Never purge it as an application-owned package.'
