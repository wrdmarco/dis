...
assert_contains "${osrm}" 'OSRM_PODMAN_STORAGE_DRIVER="vfs"'
assert_contains "${osrm}" 'OSRM_PODMAN_GRAPH_ROOT="/var/lib/containers/dis-osrm-vfs"'
assert_contains "${osrm}" 'OSRM_PODMAN_RUN_ROOT="/run/containers/dis-osrm-vfs"'
assert_contains "${osrm}" '"${OSRM_PODMAN_PATH}" "${OSRM_PODMAN_GLOBAL_ARGS[@]}"'
assert_contains "${osrm}" 'enable the Proxmox LXC nesting feature on the DIS container host and retry'
...
assert_contains 'scripts/uninstall.sh' 'for held_package in podman osrm-backend osmium-tool; do'
