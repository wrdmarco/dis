...
if [ "${ROOT_DIR}" != "${DIS_INSTALL_PATH}" ]; then
  fail "DIS must be cloned to ${DIS_INSTALL_PATH}. Current path is ${ROOT_DIR}. Run: sudo git clone <repo-url> ${DIS_INSTALL_PATH}"
fi
...
if [ ! -f "${ENV_FILE}" ]; then
  ...
  run_cmd cp "${ROOT_DIR}/.env.example" "${ENV_FILE}"
fi
...
run_cmd ln -sfn "${ENV_FILE}" "${ROOT_DIR}/webapp/backend/.env"
run_cmd chown -h "${DIS_USER}:${DIS_GROUP}" "${ROOT_DIR}/webapp/backend/.env"

log "Creating frontend production environment"
cat > "${ROOT_DIR}/webapp/frontend/.env.production" <<EOF
...
EOF
run_cmd chown "${DIS_USER}:${DIS_GROUP}" "${ROOT_DIR}/webapp/frontend/.env.production"
run_cmd chmod 0640 "${ROOT_DIR}/webapp/frontend/.env.production"

log "Generating Nginx site configuration"
GENERATED_NGINX_DIR="${ROOT_DIR}/storage/generated/nginx"
ensure_directory "${GENERATED_NGINX_DIR}" root root 0755
GENERATED_NGINX_CONF="${GENERATED_NGINX_DIR}/dis.conf"
run_cmd cp "${ROOT_DIR}/infrastructure/nginx/dis.conf" "${GENERATED_NGINX_CONF}"
run_cmd sed -i "s/server_name dis.example.nl;/server_name ${DOMAIN};/" "${GENERATED_NGINX_CONF}"

log "Provisioning database"
APP_ROOT="${ROOT_DIR}" ENV_FILE="${ENV_FILE}" bash "${ROOT_DIR}/scripts/provision-database.sh"

log "Running deployment"
APP_ROOT="${ROOT_DIR}" NGINX_SOURCE="${GENERATED_NGINX_CONF}" bash "${ROOT_DIR}/scripts/deploy.sh"

if [ "${RUN_HEALTHCHECK}" = "1" ]; then
  log "Running final local health check"
  HEALTH_URL="http://127.0.0.1/health" bash "${ROOT_DIR}/scripts/healthcheck.sh"
fi
